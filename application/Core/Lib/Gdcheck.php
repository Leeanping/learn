...
class Gdcheck {
	...
	function __construct($time=300){
		...
		$this->domain = \Core\Config::get('cookiedomain','basic',null);
		...
		$this->ckID = str_replace(array('"',"'"),'',\Core\Fun::getcookie($this->ckCkey));
...
//		$this->GetNumber();
	...
	function getNumber(){
		...
		$row = \Core\Db::fetchOne("Select * From " . self::$Table . " Where id = '" . $this->ckID . "'");
	...
	public static function check($num,$noupdate=false){
		...
		else if(\Core\Fun::time()-$self->ckCrtime>$self->ckTime)
	...
	function updatesession(){
		...
		\Core\Db::update(self::$Table, "id = '" . $this->ckID . "'", $ar);
	...
	function createImg(){
		...
		\Core\Fun::setcookie($this->ckCkey,$this->ckID,null,true);
