...
class Session
{
	...
	public static function write($id, $data)
	{
		...
		try
		{
			\Core\Db::query("Replace Into ##__base_session set {$set}", true);
		}
		catch (Exception $e)
		{
		}
		self::$sessions[$id] = $data;
		\Core\Cache::write(self::getcacheid($id), $replace , session_cache_expire() * 60 + 1, 'memcache');
	}
	...
	public static function destroy($id)
	{
		...
		try
		{
			\Core\Db::query("Delete from ##__base_session where `skey`='{$skey}'", true);
		}
		catch (Exception $e)
		{
	...
	public static function gc($max)
	{
		if (mt_rand(0, self::GCRATE) == 0)
		{
			...
			try
			{
				\Core\Db::query("Delete from `##__base_session` where `expire` < {$time}", true);
				\Core\Db::query("OPTIMIZE TABLE `##__base_session`", true);
			}
			catch (Exception $e)
			{
	...
	public static function onlinenum($sec=300)
	{
		...
		{
			$num = \Core\Cache::read('_cache/onlineuser.php', 'memcache');
			if($num)
				self::$onlinenum = $num;
			else
			{
				$expire = time() - $sec;
				self::$onlinenum = \Core\Db::getOne("Select count(*) From ##__base_session where `lastupdate`>$expire");
				\Core\Cache::write('_cache/onlineuser.php',self::$onlinenum,60, 'memcache');
			}
		}
	...
	protected static function loadsession($id=null)
	{
		...
		$tmp = \Core\Cache::read(self::getcacheid($id), 'memcache');
		...
		if (!isset(self::$sessions[$id]))
		{
			$time = time();
			$skey = \Core\Db::sqlescape($id);
			$tmp = \Core\Db::fetchOne("Select `uid`,`lastupdate`,`ip`,`expire`,`sdata` From ##__base_session where `skey`='{$skey}' and `expire`>{$time}");
			if (!$tmp)
			{
				$tmp = array();
			}
			\Core\Cache::write(self::getcacheid($id), $tmp, session_cache_expire() * 60 + 1, 'memcache');
	...
	{
		\Core\Cache::remove(self::getcacheid($id), 'memcache');
