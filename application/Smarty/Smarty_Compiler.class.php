...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_tag($template_tag)
    {
        ...
        switch ($tag_command) {
			...
			case 'vpconfig':
                ...
                break;
            case 'vpfun':
                return $this->_compile_vpfun_tag($tag_args);
                break;
            case 'vpadsingle':
    ...
    function _compile_vplist_start($tag_args)
    {
        ...
        $output = '<?php ';
		$output .= "\$_from = T('" . $from . "')->getList(array($arg));";
    ...
    function _compile_vpconfig_tag($tag_args)
    {
        ...
        $output = '<?php ';
        $output .= "echo C('" . $key . "', '" . $group . "', '" . $default . "');";
        $output .= '?>';
     ...
     * @return string
     */
    function _compile_vpfun_tag($tag_args){
        $attrs = $this->_parse_attrs($tag_args);

        if (empty($attrs['func'])) {
            return $this->_syntax_error("foreach: missing 'function' attribute", E_USER_ERROR, __FILE__, __LINE__);
        }

        $func = $this->_dequote($attrs['func']);

        if (empty($attrs['params'])) {
            return $this->_syntax_error("foreach: missing 'params' attribute", E_USER_ERROR, __FILE__, __LINE__);
        }

        $params = $this->_dequote($attrs['params']);
        ...
        if(strpos($params, ',') !== false){
            if(strpos($params, '$') !== false){
                ...
                $output .= "echo \Core\Helper::" . $func . '("';
                $outarr = [];
                foreach ($exparams as $key => $param) {
                    if(strpos($param, '$') !== false){
                        $param = str_replace('$', '', $param);
                        if(strpos($param, '.') !== false){
                            $childparam = [];
                            $childparam = explode('.', $param);
                            array_push($outarr, "{\$this->_tpl_vars[" . $childparam[0] . "][" . $childparam[1] . "]}");
                        }else{
                            array_push($outarr, "{\$this->_tpl_vars[" . $param . "]}");
                        }
                    }else{
                        array_push($outarr, $param);
                    }
                }
                $output .= implode(',', $outarr);
                $output .= '");';
            }else{
                $output .= "echo \Core\Helper::" . $func . "('" . $params . "');";
            }
        }else{
            if(strpos($params, '$') !== false){
                ...
                if(strpos($params, '.') !== false){
                    $childparam = [];
                    $childparam = explode('.', $params);
                    $output .= "echo \Core\Helper::" . $func . "(\$this->_tpl_vars['" . $childparam[0] . "']['" . $childparam[1] . "']);";
                }else{
                    $output .= "echo \Core\Helper::" . $func . "(\$this->_tpl_vars['" . $params . "']);";
                }
            }else{
                $output .= "echo \Core\Helper::" . $func . "('" . $params . "');";
            }
    ...
    function _compile_vpadsingle_tag($tag_args)
    {
        ...
        $output = '<?php ';
        $output .= "echo \Core\Fun::getAdSingle('" . $tagname . "', " . $typeid . ");";
        $output .= '?>';
