...

{
    ...
    function sqlDumpTableData($table, $startfrom = 0 ,$filesize = 2048, $backupdir = NULL,$fn=1) {
        ...
        \Core\Db::execute('SET SQL_QUOTE_SHOW_CREATE=0');
        ...
        if(preg_match('/base_session/i',$table['name'])){
            ...
            $arr = \Core\Db::fetchAll($sql);
            foreach ($arr as $key => $row) {
                if($row && strlen($tableDump) <= $ret['filesize']){
                    $tableDump .= "insert into $table[name] values (".implode(',',array_map(array('\Model\Db','fieldSlashes'),$row)).");\n";
            ...
            if($hasPk){
                ...
                $ret['last'] = intval(\Core\Db::getOne("select count(`{$table[fields][0]}`) from $table[name] where $table[pk] > $startfrom"));

            }else{

                //$sql = "select `{$table[fields][0]}` from $table[name] limit $startfrom , $offset";
                $sql = "SELECT COUNT(*) AS row_count FROM " . $table['name'] . " limit $startfrom , $offset";
                $result = \Core\Db::fetchOne($sql);
                $ret['last'] = intval($result['row_count']);
    ...
    function getTableInfo($table){
        ...
        $ret = array();
        $tables = \Core\Db::fetchAll('SHOW FULL COLUMNS FROM '.$table);
        //$query = \Core\Db::query('SHOW FULL COLUMNS FROM '.$table);
        ...
        //$tableInfo = \Core\Db::fetchArray(\Core\Db::query('show create table '.$table),MYSQL_NUM);
        $tableInfo = \Core\Db::fetchAll('show create table '.$table);

        $ret['create'] = $tableInfo[0]['Create Table'].";\n";
