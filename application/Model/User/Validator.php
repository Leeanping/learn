...
class Validator
{
    ...
    public static function checkHomeNation($nation)
    {
        if(! self::checkStrLength($nation, 0, 6))
            return 0;
        if(! empty($nation) && ! in_array($nation, array_keys(\Core\Comm\Util::getNationList())))
            return 0;
        return 1;
    }
    ...
    public static function checkHomeProvince($nation, $province)
    {
        if(! self::checkStrLength($province, 0, 6))
            return 0;
        if(! empty($province) && ! in_array($province, array_keys(\Core\Comm\Util::getProvinceList($nation))))
            return 0;
        return 1;
    }
    ...
    public static function checkHomeCity($nation, $province, $city)
    {
        if(! self::checkStrLength($city, 0, 6))
            return 0;
        if(! empty($city) && ! in_array($city, array_keys(\Core\Comm\Util::getCityList($nation, $province))))
            return 0;
        return 1;
    }
